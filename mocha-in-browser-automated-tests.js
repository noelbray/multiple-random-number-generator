    ...
    describe("function numbersToGenerate", function() {
        ...
        function returnNumber(number = 10, type = "integer", sign = "+") {
            ...
            if (type === "integer") {
                ...
                return Math.floor(Math.random() * number) * operand;
            }
            // return a floating point number

            return Math.fround(Math.random() * number) * operand;
        }

        function makeNumbersToGenerateTest( 
            ...
            it(`When mrngAllowNegativeNumbers.checked = ${allowNegatives},
            mrngOutputIntegers.checked = ${outputIntegers},
            mrngNumberOfRuns.value = ${runCount},
            mrngMinNumber.value = ${min},
            mrngMaxNumber.value = ${max},
            ${runCount} numbers should be generated in the innerText of mrngOutput paragraph.`, 
            function() {

                mrngAllowNegativeNumbers.checked = allowNegatives;
                ...
                // console.log("allowNegatives:", allowNegatives, "outputIntegers:", outputIntegers, "runCount:", runCount, "min:", min, "max:", max);

                numbersToGenerate();

                // innerText generated by numbersToGenerate
                let output = mrngOutput.innerText;

                // let numbersArray = output.match(/-*\d+(?:\.\d*)?/g) ?? 0;

                let numbersArray = output.match(/-*\d+(?:\.\d+)?(e[+|-]|e)\d+|-*\d+(?:\.\d+)?/g) ?? [];
                // This regex was created and tested here: https://regex101.com/r/bRJsnP/1. It can detect integers, floating point numbers, and scientific notation that the input type number allows.
                // The nullish coalescing operator is used just in case .match() returns null for no matches in which case an empty array will be returned to prevent the assert.equal() test from failing because null.length throws a type error, but now assert.equal([].length, 0) will not fail.

                // Instead of using a regex with match I could have done this: 
                // let numbersArray = output.split(/\s/g) or output.split(/"\n"/g);
                // numbersArray.pop(); // remove the '' array item that's at the end of the array. 

                let arrayLength = numbersArray.length;

                // console.log(numbersArray, "length", arrayLength);

                assert.equal(arrayLength, runCount, `${arrayLength} numbers were generated, but ${runCount} numbers should have been returned, generated.\n\nIf the "numbers that should have been returned" is a floating point number or a negative number, make sure the the argument for the runCount parameter of "makeNumbersToGenerateTest" is a positive whole number.\n\n`);
            });
        }
    });
